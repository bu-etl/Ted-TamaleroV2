    ...
    for irb, f_in in enumerate(in_files):
        ...
        if (not bad_run or force) and event:
            ...
            if len(missing_l1counter)>0:
                ...
                for ml1c in missing_l1counter:
                    if len(ml1c) == 4:
                        ml1c.append(-9999) #append dummy value
                    ml1, mbcid, mev, mdelta, mbcidt = ml1c
                    if mbcidt - mbcid<7:
                        print(f"Expected issue because of missing L1A dead time: {[ml1, mbcid, mev, mdelta, mbcidt]}")
                    else:
                        print(f"missing_l1counter={[ml1, mbcid, mev,mdelta,mbcidt]}")
                #for ml1,mbcid,mev,mdelta,mbcidt in missing_l1counter:
                #    if mbcidt - mbcid<7:
                #        print("Expected issue because of missing L1A dead time:", ml1, mbcid, mev,mdelta,mbcidt)
                #    else:
