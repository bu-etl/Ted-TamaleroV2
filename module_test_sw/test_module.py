...
def setup(rb, args):
    connected_modules = [i for i, mod in enumerate(rb.modules) if mod.connected]

    if args.module > 0:
        n = args.module - 1
    else:
        n = connected_modules[0]
...
def manual_threshold_scan(etroc, fifo, rb_0, args):
    ...
    etroc.baseline = max_matrix
    etroc.noise_width = noise_matrix
...
def isolate(etrocs, n):
    # From Murtaza: disDataReadout and disTrigPath should be disabled, but here they look enabled (by Daniel)
    # Updating them:
    print('Exec isolate')
    for i, e in enumerate(etrocs):
        if e.is_connected():
            e.wr_reg("disDataReadout", 0x1, broadcast=True) # Daniel = 0x0
            e.wr_reg("disTrigPath", 0x1, broadcast=True) # New (24/03/2025)
            e.wr_reg("enable_TDC", 0x0, broadcast=True) # New (24/03/2025)
            e.wr_reg("DAC", 1023, broadcast=True) # New (24/03/2025)
            e.wr_reg("TH_offset", 63, broadcast=True) # New (24/03/2025)
            if i == n:
                e.wr_reg("disDataReadout", 0x0, broadcast=True) # Daniel = 0x1 
                e.wr_reg("disTrigPath", 0x0, broadcast=True) # New (24/03/2025)
                e.wr_reg("enable_TDC", 0x1, broadcast=True) # New (24/03/2025)
                e.wr_reg("DAC", 0x0, broadcast=True) # New (24/03/2025): default value of 0
                e.wr_reg("TH_offset", 10, broadcast=True) # New (24/03/2025): default value of 10
...
def readout_tests(etroc, masked_pixels, rb_0, args, result_dir = None, out_dir = None):
    ...
    rb_0.disable_MUX64() # Disable MUX64 to avoid adding extra noise
    ...
    if args.threshold == 'auto':
        ...
        print(args.threshold, 'is not a valid option for args.threshold. Skipping threshold scans...')
...
def qinj(etroc, mask, rb, thresholds, out_dir, result_dir, args):
    ...
    if args.self_trigger: 
    	self_trigger_setup(etroc, rb, args, isolate = False, verbose = True)
        ...
        for vth in tqdm(vth_axis):
            ...
            while(not worked) and (qinjatt < 10):
                try:
                    ...
                    if args.self_trigger:
                    	kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 1)
                    #print(etroc.rd_reg('DAC', row=i, col=j)) #read vth on ETROC
                    fifo.send_QInj(count=int(args.nl1a), delay=RBL1Adelay, omit_l1a = args.self_trigger) #send Qinj pulses with L1Adelay
...
def self_trigger_setup(etroc, rb, args, isolate = False, verbose = False):
    print('Working with RB', args.rb)

    etroc.bypass_THCal()
    etroc.wr_reg('DAC', 1000, broadcast=True) #set vth on ETROC

    kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 0)
    print(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', kcu.read_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE').value())
    # input('Go?')

    if verbose:
        rb.modules[args.module].show_status()
        
    former = module.get_locked_links()
    former_trig_locked = kcu.read_node(f'READOUT_BOARD_{args.rb}.TRIG_LOCKED').value()
    etroc.wr_reg("singlePort", 0) 
    etroc.wr_reg("mergeTriggerData", 0) 
    etroc.wr_reg("triggerGranularity", 4)
    later = module.get_locked_links()
    if isolate:
        for n in range(4): 
            if rb.modules[0].ETROCs[n] != etroc:
                    etroc.wr_reg("singlePort", 0) 
                    etroc.wr_reg("mergeTriggerData", 1) 
                    etroc.wr_reg("triggerGranularity", 0)
    if verbose:
        print(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', kcu.read_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE').value())
        rb.modules[args.module].show_status()
        for n in range(4): 
            print(rb.modules[0].ETROCs[n] == etroc)
            print(n, rb.modules[0].ETROCs[n].rd_reg("singlePort")) 
            print(n, rb.modules[0].ETROCs[n].rd_reg("mergeTriggerData")) 
            print(n, rb.modules[0].ETROCs[n].rd_reg("triggerGranularity")) 

    lpgpt1_changes = [e for e in former['locked'][0] if not e in later['locked'][0]]
    lpgpt2_changes = [e for e in former['locked'][1] if not e in later['locked'][1]]

    if len(lpgpt1_changes) > 1 and len(lpgpt2_changes) > 1:
        print('Something is wrong.', lpgpt1_changes, 'on lPGBT 1 and', lpgpt2_changes, 'on lPGBT2 both disconnected. Terminating...')
        adsafd
    elif len(lpgpt1_changes) == 1:
        trig_uplink_sel = 0
        link = lpgpt1_changes[0]
    elif len(lpgpt2_changes) == 1:
        trig_uplink_sel = 1
        link = lpgpt2_changes[0]
    else:
        print('Something is wrong.', lpgpt1_changes, 'on lPGBT 1 and', lpgpt2_changes, 'on lPGBT2 both disconnected. Terminating...')
        adsafd

    # input('Just set ETROC singlePort, mergeTriggerData, etc. Set TRIG_UPLINK_SEL?')
    kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_UPLINK_SEL', trig_uplink_sel)
    time.sleep(2)
    curr_trig_locked = kcu.read_node(f'READOUT_BOARD_{args.rb}.TRIG_LOCKED').value()

    print('Attempting to lock on flashing bit...')
    locked = curr_trig_locked == ((0b1 << link) | former_trig_locked)
    counter = 0
    while not locked and counter < 5:
        if curr_trig_locked == ((0b1 << link) | former_trig_locked):
            print('Successfully locked on flashing bit!')
        else:
            print('Could not lock on flashing bit.')
            print('Former TRIG_LOCKED:', bin(former_trig_locked))
            print('Current TRIG_LOCKED:', bin(curr_trig_locked))
            print('Expected Elink to lock:', bin(0b1 << link))
        counter += 1
        time.sleep(2)
        locked = curr_trig_locked == ((0b1 << link) | former_trig_locked)
    if locked:
        ...
        print('Unable to lock on flashing bit after too many tries. Terminating...')
        asdf
    # input('Just set TRIG_UPLINK_SEL and locked. Set TRIG_ENABLE_MASK_?')
    for i in range(7): 
        kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE_MASK_{i}', 0xffffffff)
    
    if verbose:
        for i in range(7): 
            print(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE_MASK_{i}', kcu.read_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE_MASK_{i}').value())
    kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_CLEAR', 0x0fffffff)
    # kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_CLEAR', 0x1)
    # input('Just set TRIG_ENABLE_MASK_ and locked. Set TRIG_ELINK_ENABLE?')
    if isolate:
        kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ELINK_ENABLE', 0x0)
        kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ELINK_ENABLE', (0b1 << link))
    else:
        kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ELINK_ENABLE', curr_trig_locked)
    # input('Just set TRIG_ELINK_ENABLE and locked. Set SELF_TRIG_RESET?')
    kcu.write_node(f'READOUT_BOARD_{args.rb}.SELF_TRIG_RESET', 1)
    time.sleep(2)
    kcu.write_node(f'READOUT_BOARD_{args.rb}.SELF_TRIG_RESET', 0)
    time.sleep(2)
    # input('Just set SELF_TRIG_RESET and locked. Reset TRIG_UPLINK_SEL?')
    kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_UPLINK_SEL', trig_uplink_sel)

    if verbose:
        check_and_clear(kcu, args)

def check_and_clear(kcu, args):
    print(kcu.read_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT').value(), 'events already in detected. Clearing...')
    kcu.write_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT_RESET', 1)
    time.sleep(5)
    kcu.write_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT_RESET', 0)
    print(kcu.read_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT').value(), 'events detected.')

def run_self_trigger(etroc, rb, thresholds, out_dir, result_dir, args):
    ...
    print(f'No. of Injections: {nPulses}') #Maybe switch to argument, using args.nl1a?
    print(f'Pixel Row: {args.row} Col: {args.col}')
    print(f'Charge Injection Delay: {delay}')
    print(f'L1A Injection Delay (ETROC Side): {L1Adelay}')
    print(f'Self-Trigger Injection Delay (KCU Side): {L1Adelay}')

    print(f"\n - Will send QInj {nPulses} pulse(s) with delay of {delay} cycles and charge of {q} fC")
    print(f"\n - to pixel at Row {i}, Col {j}.")
    print()
    worked = False
    qinjatt = 0
    while(not worked) and (qinjatt < 10):
        try:
            ...
            if len(thresholds) > 0:
                etroc.wr_reg('DAC', thresholds[i, j], row=i, col=j, broadcast=False)
            else:
                etroc.wr_reg('DAC', args.sftg_qinj, row=i, col=j, broadcast=False)
            check_and_clear(kcu, args)
            kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 1)
            fifo.send_QInj(count=nPulses, delay=RBL1Adelay, omit_l1a = True)
            kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 0)
            result = fifo.pretty_read(df)
            worked = True
        except KeyboardInterrupt:
    ...
    word_hits = 0
    tot_events = kcu.read_node('READOUT_BOARD_0.EVENT_CNT').value()
    for word in result:
        if(word[0] == 'trailer'):
            word_hits+=word[1]['hits']
        
    print(tot_events, 'events detected.')
    print(word_hits, 'data words detected.')
    print()
    kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 0)
...
if __name__ == '__main__':
    ...
    argParser.add_argument('--module', action='store', type = int, default=0, choices=[1,2,3], help="Module to test")
    ...
    argParser.add_argument('--etrocs', action = 'store', nargs = '*', type = int, default = [0, 1, 2, 3], help = 'ETROC(s) to use on a multi-ETROC module')
    ...
    argParser.add_argument('--self_trigger', action='store_true', help="Use self-trigger for qinj")
    argParser.add_argument('--sftg_qinj', action='store', type = int, default = 500, help="Use self-trigger for qinj")

    
    # Others
    argParser.add_argument('--timing_scan', action='store_true', help="Perform L1Adelay timing scan")
    argParser.add_argument('--powerboard_pause', action='store_true', help="Pause after turining on power before continuing")
    ...
    if args.test_chip:
        ...
        print(f"Module to test {module.i}")
        for i, etroc in enumerate(etrocs):
            if i not in args.etrocs:
                continue
            if not i in args.etrocs:
                continue
            if etroc.is_connected():
                print(f"Testing ETROC {etroc.chip_no} on Module {etroc.module_id} now (slot {module.i})")
                mask = masks[i]
                if not args.skip_sanity_checks:
                    etroc.pixel_sanity_check()
                isolate(etrocs, i)
                etroc.wr_reg('Bypass_THCal', 1, broadcast=True)
                ...
                print("Module 1:", rb.DAQ_LPGBT.read_gpio('LPGBT.RWF.PIO.PIOOUTH', 6))
                print("Module 2:", rb.TRIG_LPGBT.read_gpio('LPGBT.RWF.PIO.PIOOUTH', 6))
                print("Module 3:", rb.TRIG_LPGBT.read_gpio('LPGBT.RWF.PIO.PIOOUTL', 5))
                if args.qinj:
                    qinj(etroc, mask, rb, thresholds, out_dir, result_dir, args)

                if args.self_trigger and not (args.qinj or args.timing_scan):
                    run_self_trigger(etroc, rb, [], out_dir, result_dir, args)
            
                if args.timing_scan:
                    print(f"Timing scan on ETROC {etroc.chip_no} on Module {etroc.module_id} now")
                    ...
                    if args.self_trigger: 
                       vth_tscan = args.sftg_qinj
                    else:
                       vth_tscan = np.max(thresholds) + 30
                    print(f'Highest threshold found at {np.max(thresholds)} so we will set the threshold at {vth_tscan}')
                    print(f'Charge default for timing scan set at 30 fC')
                    charges = [30]
                    ...
                    if args.self_trigger:
                    	kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 0)
                    	check_and_clear(kcu, args)
                    before = kcu.read_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT').value()
                    for L1Adelay in tqdm(range(0, 511)): #Max allowed L1ADelay for ETROC is 511
                        for RBL1Adelay in range(0, 5):
                            if RBL1Adelay < 0:
                            	continue
                            worked = False
                            qinjatt = 0
                            while(not worked) and (qinjatt < 10):
                                try:
                                    etroc.QInj_set(charges[0], delay, L1Adelay, row=i, col=j, broadcast = False) #set reg on ETROC
                                    if args.self_trigger:
                                        kcu.write_node(f'READOUT_BOARD_{args.rb}.TRIG_ENABLE', 1)
                                    etroc.wr_reg('DAC', int(vth_tscan), row=i, col=j, broadcast=False) #set vth on ETROC
                                    #print(etroc.rd_reg('DAC', row=i, col=j)) #read vth on ETROC
                                    fifo.send_QInj(count=int(30), delay=RBL1Adelay, omit_l1a = args.self_trigger) # 320 pulses by default
                                    #check_and_clear(kcu, args)
                                    result = fifo.pretty_read(df)
                                    worked = True
                                    eve =  kcu.read_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT').value() - before
                                    before = kcu.read_node(f'READOUT_BOARD_{args.rb}.EVENT_CNT').value()
                                except KeyboardInterrupt:
                                    print(f'Interupted by keyboard. Terminating run')
                                    raise
                            ...
                            for word in result:
                                if(word[0] == 'trailer'):
                                    hits+=word[1]['hits']
                            # print(f'L1Adelay set to {L1Adelay} and RBL1Adelay set to {RBL1Adelay}, obtaining {int(hits)} hits')
                            results[0].append(int(hits))
                            results[1].append(int(eve))
                            L1Adelay_scan.append((L1Adelay, RBL1Adelay))
                            if hits > 0:
                            	print(f'{hits} found at {(L1Adelay, RBL1Adelay)}')
                    print(results)
