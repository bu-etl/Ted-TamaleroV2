...
def HitsvDACplotter(df, pa, args, charge = None, mode = 'pre'):
    ...
    hep.cms.label(rlabel=f'{pa["loc_title"]}', llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
    # plt.title(f'Hits v. Threshold DAC for {pa["loc_title"]}, \n# of L1A triggers: {args.nl1a}', fontsize = pa['titfontsize'])
    ...
    if not charge:
        for q in tqdm(np.unique(df.charge)):
            ...
            x = df.vth[idx]
            y = df.hits[idx]
            plt.plot(x, y, 'o-', label = f'Qinj = {q} fC')
    ...
    plt.savefig(savename.replace('pdf', 'png'), bbox_inches = 'tight')
    if args.show_plots:
        plt.show()
    plt.close()
...
def plotFallingEdge(df, pa, args):
    ...
    plt.legend()
    plt.savefig(f'{pa["store"]}/Threshold_DAC_Limit.png', bbox_inches = 'tight')
    plt.savefig(f'{pa["store"]}/Threshold_DAC_Limit.png', bbox_inches = 'tight')
    if args.show_plots:
        plt.show()
    plt.close()
...
def plotSingleVthDists(df, pa, args):
    print('Working on TOT, TOA, and Cal dists for individual Threshold DAC settings')
    for q in tqdm(np.unique(df.charge)):
        for d in tqdm(args.plotted_vths, leave = False, desc = f'Working on QSel = {q}'):
            ...
            for att in ['tot', 'cal', 'toa']:
                fig = plt.figure(figsize = pa['figsize'])
                hep.cms.label(rlabel=pa["loc_title"], llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
                data = df[att][idx].iloc[0]
                plt.hist(data, bins = args.nbins, density = True)#bins = range(np.min(tot), np.max(tot) + 1), density = True)
                # plt.title(f'{att.upper()} Codes for Threshold DAC = {d} {pa["loc_title"]}\nEntries: {args.nl1a}, Qinj = {q} fC')
                plt.xlabel(f'{att.upper()} Values')
                plt.ylabel('Frequency')
                plt.yscale('log')
                plt.savefig(f'{pa["store"]}/{att.upper()}_vth_{d}_q{q}.pdf', bbox_inches = 'tight')
                plt.savefig(f'{pa["store"]}/{att.upper()}_vth_{d}_q{q}.png', bbox_inches = 'tight')
                plt.close()


def plotCodesvDAC(df, pa, args, code):
    print('Working on TOA, TOT, and CAL v. DAC')
    ...
    for q in tqdm(np.unique(df.charge)):
        ...
        for att in atts:
            ...
            hep.cms.label(rlabel=f'{pa["loc_title"]}, Qinj = {q} fC', llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
            ...
            # plt.title(f'Mean {att.upper()} {mode} vs. Theshold DAC  {pa["loc_title"]}, Qinj = {q} fC', fontsize = pa['titfontsize'])
            bar = 0.15*ymax
            #plt.ylim([ymin - bar, ymax + bar])
            plt.savefig(f'{pa["store"]}/DAC_v_{att.upper()}{mode}_q{q}.pdf', bbox_inches = 'tight')
            plt.savefig(f'{pa["store"]}/DAC_v_{att.upper()}{mode}_q{q}.png', bbox_inches = 'tight')
            plt.close()
        
        fig = plt.figure(figsize = pa['hist2dsize'])
        hep.cms.label(rlabel=f'{pa["loc_title"]}\nQinj = {q} fC', llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
        x = []
        y = []
        for i in range(len(toa)):
            x += toa.iloc[i]
            y += tot.iloc[i]
        _, _, _, img = plt.hist2d(x, y, norm = 'log', bins = [50, 50])
        plt.colorbar(img)
        plt.xlabel(f'TOA {mode}')
        plt.ylabel(f'TOT {mode}')
        plt.savefig(f'{pa["store"]}/TOA_v_TOT{mode}_q{q}.pdf', bbox_inches = 'tight')
        plt.savefig(f'{pa["store"]}/TOA_v_TOT{mode}_q{q}.png', bbox_inches = 'tight')
        plt.close()
    ...
    for att in atts:
        ...
        for q in tqdm(np.unique(df.charge)):
            ...
            plt.errorbar(data[q]['vth'], data[q][att + 'avg'], data[q][att + 'std'], fmt = 'o-', capsize = 3, label = f'Qinj = {q} fC')
        plt.xlabel('Threshold DAC', fontsize = pa['labfontsize'])
        plt.ylabel(f'{att.upper()} {mode} Mean', fontsize = pa['labfontsize'])
        # plt.title(f'Mean {att.upper()} vs. Theshold DAC  {pa["loc_title"]}', fontsize = pa['titfontsize'])
        plt.legend()
        bar = 0.15*ymax
        #plt.ylim([ymin - bar, ymax + bar])
        plt.savefig(f'{pa["store"]}/DAC_v_{att.upper()}{mode}.pdf', bbox_inches = 'tight')
        plt.savefig(f'{pa["store"]}/DAC_v_{att.upper()}{mode}.png', bbox_inches = 'tight')
        if args.show_plots:
            plt.show()
        plt.close()
    ...
    for i in range(len(df)):
        x += df.toa.iloc[i]
        y += df.tot.iloc[i]
    _, _, _, img = plt.hist2d(x, y, bins = [50, 50], norm = 'log')
    plt.colorbar(img)
    plt.xlabel(f'TOA {mode}')
    plt.ylabel(f'TOT {mode}')
    plt.savefig(f'{pa["store"]}/TOA_v_TOT{mode}.pdf', bbox_inches = 'tight')
    plt.savefig(f'{pa["store"]}/TOA_v_TOT{mode}.png', bbox_inches = 'tight')
    plt.close()
...
def plotTOASDvDAC(df, pa, args):
    ...
    for q in tqdm(np.unique(df.charge)):
        ...
        plt.xlabel('Threshold DAC', fontsize = pa['labfontsize'])
        plt.ylabel('TOA Standard Deviation [ns]', fontsize = pa['labfontsize'])
        # plt.title(f'TOA Standard Deviation vs. Theshold DAC\n  {pa["loc_title"]}, Qinj = {q} fC', fontsize = pa['titfontsize'])
        plt.ylim(top = 0.1)
        plt.savefig(f'{pa["store"]}/DAC_v_TOASD_q{q}.pdf', bbox_inches = 'tight')
        plt.savefig(f'{pa["store"]}/DAC_v_TOASD_q{q}.png', bbox_inches = 'tight')
        plt.close()
        ...
        plt.savefig(f'{pa["store"]}/DAC_v_TOACODESD_q{q}.pdf', bbox_inches = 'tight')
        plt.savefig(f'{pa["store"]}/DAC_v_TOACODESD_q{q}.png', bbox_inches = 'tight')
        plt.close()
    ...
    hep.cms.label(rlabel=f'{pa["loc_title"]}', llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
    for q in tqdm(np.unique(df.charge)):
        plt.plot(data[q]['vth'], data[q]['toastd'], 'o-', label = f'Qinj = {q} fC')
    plt.xlabel('Threshold DAC', fontsize = pa['labfontsize'])
    plt.ylabel('TOA Standard Deviation [ns]', fontsize = pa['labfontsize'])
    # plt.title(f'TOA Standard Deviation vs. Theshold DAC\n  {pa["loc_title"]}', fontsize = pa['titfontsize'])
    plt.legend()
    plt.ylim([0, 0.1])
    plt.savefig(f'{pa["store"]}/DAC_v_TOASD.pdf', bbox_inches = 'tight')
    plt.savefig(f'{pa["store"]}/DAC_v_TOASD.png', bbox_inches = 'tight')
    if args.show_plots:
        plt.show()
    plt.close()
    ...
    hep.cms.label(rlabel=f'{pa["loc_title"]}', llabel = 'ETL Preliminary',  fontsize = pa['titfontsize'] )
    for q in tqdm(np.unique(df.charge)):
        plt.plot(data[q]['vth'], data[q]['toacodestd'], 'o-', label = f'Qinj = {q} fC')
    plt.xlabel('Threshold DAC', fontsize = pa['labfontsize'])
    plt.ylabel('TOA Code Standard Deviation', fontsize = pa['labfontsize'])
    # plt.title(f'TOA Code Standard Deviation vs. Theshold DAC\n  {pa["loc_title"]}', fontsize = pa['titfontsize'])
    plt.legend()
    plt.savefig(f'{pa["store"]}/DAC_v_TOACODESD.pdf', bbox_inches = 'tight')
    plt.savefig(f'{pa["store"]}/DAC_v_TOACODESD.png', bbox_inches = 'tight')
    if args.show_plots:
        plt.show()
    plt.close()
...
if args.loc == 'broadcast':
    ...
    loc_title = f'{args.etroc} Row {i} Col {j}'
    pix_path = f'/r{i}c{j}/'
