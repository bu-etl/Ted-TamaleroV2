...
class ETROC():
    ...
    def get_locked_links(self) -> List[int]:
        self.get_elink_status()
        locked_elinks = []
        for i in [0, 1]:
            for j, link in enumerate(self.elinks[i]):
                if self.links_locked[i][j]:
                    locked_elinks.append(link)
        return locked_elinks
    
    def get_lpgbt_from_link(self, link: int) -> int:
        """
        Returns the lpGBT number (0 or 1) that the given link belongs to.
        Raises ValueError if the link is not found in any lpGBT.
        """
        for i in [0, 1]:
            if link in self.elinks[i]:
                return i
        raise ValueError(f"Link {link} not found in any lpGBT")
    
    # =======================
    # === HIGH-LEVEL FUNC ===
    # =======================

    def configure_self_trig(self, refresh_self_trig: bool = False):
        self.rb.self_trig_stop()              # disable self trigger fw module
        if refresh_self_trig:
            print("Refreshing self trigger, note this will unconfigure any previously configured self trigger settings.")
            self.rb.self_trig_refresh()          # empties all self trig registers on fw (WARNING: Resets all other chips set up for self trigger)
        print("caching DAC values...")
        dac = np.empty([16, 16])                 # save all dac values to reapply at the end
        for pix, _ in np.ndenumerate(dac):
            dac[pix] = self.rd_reg("DAC", row=pix[0], col=pix[1])

        self.bypass_THCal()                      # needed to set DAC value manually
        self.wr_reg("DAC", 1023, broadcast=True) # make etroc quiet as possible
        #self.rb.self_trig_clear(0x0fffffff)      # clears all self trigger bits

        elinks_locked_before = self.get_locked_links()    
        self.wr_reg("singlePort", 0)             # ETROC SETUP: have data go to right port and trigger go to left port, 
        self.wr_reg("mergeTriggerData", 0)       # hardware wired such that they go to different lpgbts
        self.wr_reg("triggerGranularity", 4)     # set trigger data size is 8 bits
        self.wr_reg("serRateRight", 0)           # These steps will "unlock" a data elink 
        self.wr_reg("serRateLeft", 0)            # now we will find the one that unlocked
        elinks_locked_after = self.get_locked_links()
        unlocked_data_links = set(elinks_locked_before) - set(elinks_locked_after)
        if hasattr(self, "self_trigger_link"):
            # if this was already found use it!
            # users can also manually set the attribute
            print("reusing link", self.self_trigger_link)
            ...
        elif len(unlocked_data_links) > 1:
            raise Exception("You lost more than one data link after setting up self trigger, aborting as we do not know the one used for trigger...")
        elif len(unlocked_data_links) == 0:
            raise Exception("No data links were unlocked after setting up self trigger data link.")
        else:
            self.self_trigger_link = list(unlocked_data_links)[0]
        lpgbt_number = self.get_lpgbt_from_link(self.self_trigger_link)
        print(f"Unlocked Link: {self.self_trigger_link} and lpgbt number is {lpgbt_number}. [{elinks_locked_before=}, {elinks_locked_after=}]")
        
        self.rb.self_trig_uplink_sel(lpgbt_number)   # select the lpgbt used for trigger uplink
        time.sleep(2)
        t0 = time.perf_counter()
        while not self.rb.self_trig_locked(self.self_trigger_link):
            time.sleep(1)
            if time.perf_counter() - t0 > 30:
                raise ValueError(f"This selected link ({self.self_trigger_link}) was not locked for self trigger after {time.perf_counter() - t0} seconds")
            else:
                print("Waiting for flashing bits to clear...")
        print("Flashing bits cleared! Link is locked.")
        # if self.rb.self_trig_locked(self.self_trigger_link):
        #     raise ValueError(f"This selecfted link ({self.self_trigger_link}) was not locked for self trigger.")
        # else:
        #     print(f"Flashing bits cleared! Link is locked.") 
        self.rb.self_trig_enable_mask(0xffffffff)    # if we want per etroc granularity this needs to be updated
        self.rb.self_trig_clear(0x0fffffff)          # clear flashing bits on locked trigger elinks
        self.rb.self_trig_elink_enable(self.self_trigger_link) # enable the link used for self trigger
        self.rb.self_trig_soft_reset()               # toggles SELF_TRIG_RESET block
        self.rb.self_trig_uplink_sel(lpgbt_number)   # select the lpgbt used for trigger uplink

        # re apply dac values
        print("Re applying cached DAC values...")
        from tqdm import tqdm
        with tqdm(total=256, bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}') as pbar:
            for pix, dac_value in np.ndenumerate(dac):
                time.sleep(0.005)
                dac[pix] = self.wr_reg("DAC", int(dac_value), row=pix[0], col=pix[1])
                pbar.update()
    ...
    def run_threshold_scan(self, offset='auto', use=True, out_dir=None):
        print('Executing run_threshold_scan()...')
        # isolate
        #print(self.rd_reg('Bypass_THCal', row=1, col=1))
        print('Isolating ETROC and setting max values for DAC and TH_offset')
        self.wr_reg('Bypass_THCal', 1, broadcast=True)
        self.wr_reg("disDataReadout", 0x1, broadcast=True) # Daniel = 0x0
        self.wr_reg("disTrigPath", 0x1, broadcast=True) # New (24/03/2025)
        self.wr_reg("enable_TDC", 0x0, broadcast=True) # New (24/03/2025)
        self.wr_reg("DAC", 1023, broadcast=True) # New (24/03/2025)
        self.wr_reg("TH_offset", 63, broadcast=True) # New (24/03/2025)
        #self.wr_reg('Bypass_THCal', 1, broadcast=True)
        #
        from tqdm import tqdm
        ...
        with tqdm(total=256, bar_format='{l_bar}{bar:20}{r_bar}{bar:-20b}') as pbar:
            for pixel in range(256):
                ...
                pbar.update()
        #
        print('Enabling data readout and trigger path...')
        self.wr_reg("disDataReadout", 0x0, broadcast=True) # Daniel = 0x1 
        self.wr_reg("disTrigPath", 0x0, broadcast=True) # New (24/03/2025)
        self.wr_reg("enable_TDC", 0x1, broadcast=True) # New (24/03/2025)
        #
        ...
        for row in range(16):
            for col in range(16):
                if int(thresholds[row][col]) > 1023:
                    print('Bad value (', int(thresholds[row][col]), ')! Setting to 1023')
                    self.wr_reg('DAC', 1023, row=row, col=col)
                else:
                    self.wr_reg('DAC', int(thresholds[row][col]), row=row, col=col) # want to get some noise
    ...
    def plot_threshold(self, outdir='../results/', noise_width=False):
        ...
        ax.text(1.0, 1.01, f"Module {self.module_id}", fontsize=24, color='black', horizontalalignment='right', verticalalignment='bottom', transform=ax.transAxes)
        ...
        ax.hist(matrix.flatten(), bins= np.linspace(0, 16, 17) if noise_width else 20, histtype='step', edgecolor='tab:blue', linewidth=2)
        if noise_width:
        ...
        if noise_width:
            fig.savefig(f'{outdir}/module_{self.module_id}_etroc_{self.chip_no}_noise_width_histogram.png')
        else:
            fig.savefig(f'{outdir}/module_{self.module_id}_etroc_{self.chip_no}_baseline_histogram.png')
    ...
    def auto_threshold_scan(self, row=0, col=0, broadcast=False, offset='auto', time_out=3, verbose=False, use=True):
        ...
        self.wr_reg('RSTn_THCal', 0, row=row, col=col, broadcast=broadcast) # Check with Murtaza: Needed?
        self.wr_reg('RSTn_THCal', 1, row=row, col=col, broadcast=broadcast) # Check with Murtaza: Needed?
        time.sleep(0.01)
        ...
        self.wr_reg('DAC', 1023, row=row, col=col, broadcast=broadcast)
        self.wr_reg('TH_offset', 63, row=row, col=col, broadcast=broadcast)
...
#        return self.rd_reg('DAC', row=row, col=col)
    ...
    def reset_Power(self):
        print("Resetting power sequencer controller by toggling softBoot")
