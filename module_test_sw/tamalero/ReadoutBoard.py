...
class ReadoutBoard:
    ...
    def read_rb_thermistor(self, rt):
        ...
        if rt==1:
            ...
            elif self.ver == 4 or self.ver == 5: # Expected no changes between these two versions 
                vref = 1.2
                self.MUX64.select_channel(63) # Ground
                rt2_voltage = self.DAQ_LPGBT.read_adc(5)/(2**10-1) # FIXME 5 shouldn't be hardcoded
                return get_temp(rt2_voltage, vref, 10000, 25, 10000, 3900)  # this comes from the lpGBT ADC
            else:
                raise Exception("Unknown lpgbt version")
    ...
    def disable_MUX64(self):
        if self.ver > 3:
            self.MUX64.select_channel(63) # Hardcoded
            print("MUX64 connected to ground")
        else:
            print("Ground not connected for MUX64 in this version of the Readout Board: Skipping...")

    # Functions to select or not select the module slot which
    # we want to configure
    # Positions [0,1,2] for 3 module RB
    # (RBF or later only)
    ...
    ###
    #### SELF TRIGGER METHODS
    ###
        
    def self_trig_soft_reset(self) -> None:
        """
        Toggles the SELF_TRIG_RESET function.
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.SELF_TRIG_RESET",1)
        time.sleep(1)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.SELF_TRIG_RESET",0)

    def self_trig_stop(self) -> None:
        """
        Stops self triggering mode, through the KCU register TRIG_ENABLE
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ENABLE", 0)

    def self_trig_start(self) -> None:
        """
        Starts self triggering mode, through the KCU register TRIG_ENABLE
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ENABLE", 1)

    def self_trig_clear(self, value: int) -> None:
        """
        Clear flashing bits on locked trigger elinks
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_CLEAR", value)

    def self_trig_enable_mask(self, value: int) -> None:
        """
        Bitmask to enable bits in the trigger link
        """
        # if we want per etroc granularity this needs to be more specific
        for i in range(7):
            self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ENABLE_MASK_{i}", value) 

    def self_trig_elink_enable(self, link: int) -> None:
        """
        Enable ETROC elink for self trigger
        """
        current = self.kcu.read_node(f"READOUT_BOARD_{self.rb}.TRIG_ELINK_ENABLE").value()
        # Set the bit at position 'link' to 1
        updated = current | (1 << link)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ELINK_ENABLE", updated)

    def self_trig_uplink_sel(self, value: int) -> None:
        """
        Select uplink 0 or 1 to input as to self trrigger
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_UPLINK_SEL", value)

    def self_trig_locked(self, link: int) -> bool:
        """
        Returns true if the link is locked or not.
        Uses TRIG_LOCKED register: 28 bit long, when index value = 1 link is locked (found flashing bits)
        """
        locked_links = self.kcu.read_node(f'READOUT_BOARD_{self.rb}.TRIG_LOCKED').value()
        # Check if the bit at position 'link' is not 0 
        return (locked_links & (1 << link)) != 0

    def self_trig_refresh(self) -> None:
        """
        Returns all the registers for self trigger to the defualt value
        """
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ENABLE", 0)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_CLEAR", 0)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ELINK_ENABLE", 0)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_UPLINK_SEL", 0)
        for i in range(7):
            self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_ENABLE_MASK_{i}", 0)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.SELF_TRIG_RESET",0)
        self.kcu.write_node(f"READOUT_BOARD_{self.rb}.TRIG_DLY_SEL", 0)

    def self_trig_status(self):
        kcu_regs = [
            f"READOUT_BOARD_{self.rb}.TRIG_ENABLE",
            f"READOUT_BOARD_{self.rb}.TRIG_CLEAR",
            f"READOUT_BOARD_{self.rb}.TRIG_ELINK_ENABLE",
            f"READOUT_BOARD_{self.rb}.TRIG_UPLINK_SEL",
            # f"READOUT_BOARD_{self.rb}.SELF_TRIG_RESET", # unreadable
            f"READOUT_BOARD_{self.rb}.TRIG_DLY_SEL",
            f'READOUT_BOARD_{self.rb}.TRIG_LOCKED'
        ]
        kcu_regs += [f"READOUT_BOARD_{self.rb}.TRIG_ENABLE_MASK_{i}" for i in range(7)]

        for r in kcu_regs:
            print(r, hex(self.kcu.read_node(r).value()))