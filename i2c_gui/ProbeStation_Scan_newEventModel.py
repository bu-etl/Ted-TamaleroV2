...
def run_ProbeStation(
    ...
    if(do_qinj): qinj_dir.mkdir(exist_ok=False)
    qinj_dir_short = f'{comment_str}_{wafer_name}_{chip_name}_R{row}_C{col}_qinj'
    i2c_log_dir = data_dir / f'{comment_str}_{wafer_name}_{chip_name}_R{row}_C{col}_I2C'
    if(do_i2c_check): i2c_log_dir.mkdir(exist_ok = False)
    ...
    if(do_pll):
        ...
        chip.write_decoded_value("ETROC2", "Peripheral Config", "asyStartCalibration")
        print(f"PLL Calibration for chip: {hex(chip_address)}")
    ...
    if do_baseline:
        ...
        while ScanDone_handle.get() != "1":
            ...
            chip.read_all_block("ETROC2", "Pixel Status")
            retry_counter += 1
            if retry_counter == 1000 and ScanDone_handle.get() != "1":
    ...
    if do_qinj:
        ...
        chip.write_decoded_value("ETROC2", "Peripheral Config", "asyAlignFastcommand")
        print(f"FC Calibration done for chip: {hex(chip_address)}")

        ## Run DAQ
        print("Running TDC DAQ with QInj for Selected Pixel...")
        parser = parser_arguments.create_parser()
        (options, args) = parser.parse_args(args=f"-f --useIPC --hostname {fpga_ip} -t 2 -o {qinj_dir_short} -v -w -s 0x0000 -p {polarity} -d 0x1800 -a 0x0001 --clear_fifo --check_valid_data_start --start_DAQ_pulse --stop_DAQ_pulse".split())
        IPC_queue = multiprocessing.Queue()
        process = multiprocessing.Process(target=run_script.main_process, args=(IPC_queue, options, str(qinj_dir/f"probe_daq_output")))
        process.start()
